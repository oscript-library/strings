...
//  ИсключаемыеСтрокиСтекаВызовов			 - Массив	 - массив строк-исключений, эти строки и последующие строки исключаются
//  МинимальноеПроверяемоеКоличествоСтрок	 - Число	 - количество строк, которые не проверяются на исключения 
// 
// Возвращаемое значение:
//  Строка - строка, в которой удалены подстроки, указанные в массиве настроек по ключу "ИсключаемыеСтрокиСтекаВызовов"
//
Функция ПолучитьСообщениеБезСтекаВызовов(Знач Сообщение, Знач ИсключаемыеСтрокиСтекаВызовов, 
		Знач МинимальноеПроверяемоеКоличествоСтрок = 2) Экспорт

	Если ПустаяСтрока(Сообщение) Или Не ЗначениеЗаполнено(ИсключаемыеСтрокиСтекаВызовов) Тогда
		Возврат Сообщение;		
	КонецЕсли;

	МассивПодстрок = РазложитьСтрокуВМассивПодстрок(Сообщение, Символы.ПС, Ложь);
	МассивРезультат = Новый Массив;
	
	Для Счетчик = 0 По Мин(МинимальноеПроверяемоеКоличествоСтрок - 1,  МассивПодстрок.ВГраница()) Цикл
		ОчереднаяСтрока = МассивПодстрок[Счетчик];
		МассивРезультат.Добавить(ОчереднаяСтрока);
	КонецЦикла;
	ИсключилиСтроки = Ложь;
	Для Счетчик = МинимальноеПроверяемоеКоличествоСтрок По МассивПодстрок.ВГраница() Цикл
		ОчереднаяСтрока = МассивПодстрок[Счетчик];
		Если ЕстьВИсключаемойКоллекции(ОчереднаяСтрока, ИсключаемыеСтрокиСтекаВызовов) Тогда
			ИсключилиСтроки = Истина;
			Прервать;
		КонецЕсли;
		МассивРезультат.Добавить(ОчереднаяСтрока);
	КонецЦикла;
	
	Если Не ИсключилиСтроки Тогда
		Возврат Сообщение;
	КонецЕсли;
	
	Результат = СтрокаИзМассиваПодстрок(МассивРезультат, Символы.ПС);
	Возврат Результат;
КонецФункции
...
//  КоллекцияДляПоиска	 - Массив	 - коллекция, в которой ищем элементы по простому совпадению или шаблону со звездочками
// 
// Возвращаемое значение:
...
// Прочитать файл json. в файле разрешено использовать комментарии //
//
// Параметры:
//   ИмяФайла - Строка - путь файла
//   ЧитатьВСоответствие - Булево - результа будет соответствием? По умолчанию Ложь
//
//  Возвращаемое значение:
//   Соответствие, Структура - коллекция из файла
//
...
// Прочитать строку json. в файле разрешено использовать комментарии //
//
// Параметры:
//   JsonСтрока - Строка - строка json
//   ЧитатьВСоответствие - Булево - результа будет соответствием? По умолчанию Ложь
//
//  Возвращаемое значение:
//   Соответствие, Структура - коллекция из файла
//
Функция ПрочитатьСтрокуJSON(Знач JsonСтрока, Знач ЧитатьВСоответствие = Ложь) Экспорт
	JsonСтрока = УбратьКомментарииИзJsonСтроки(JsonСтрока);
...
// Прочитать файл с указанной кодировкой. По умолчанию кодировка UTF8
//
// Параметры:
//   ИмяФайла - Строка - путь файла
//   Кодировка - КодировкаТекста - кодировка. по умолчанию КодировкаТекста.UTF8
//
//  Возвращаемое значение:
//   Строка - текст файла
//
